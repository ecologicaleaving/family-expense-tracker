...
@TableIndex(
    name: 'offline_expenses_user_status_idx', columns: {#userId, #syncStatus})
@TableIndex(
    name: 'offline_expenses_user_created_idx',
    columns: {#userId, #localCreatedAt})
class OfflineExpenses extends Table {
  ...
  TextColumn get reimbursementStatus =>
      text().withDefault(const Constant('none')).check(
          reimbursementStatus.isIn(['none', 'reimbursable', 'reimbursed']))();
  ...
  TextColumn get recurringExpenseId =>
      text().nullable()(); // References recurring_expenses(id)
  BoolColumn get isRecurringInstance => boolean().withDefault(const Constant(
      false))(); // Whether this expense was auto-generated from a template

  // Receipt Image Reference (if uploaded offline)
  TextColumn get localReceiptPath => text().nullable()(); // Local file path
  IntColumn get receiptImageSize =>
      integer().nullable()(); // Bytes, for SC-012 tracking

  // Sync Metadata
  TextColumn get syncStatus =>
      text()(); // 'pending', 'syncing', 'completed', 'failed', 'conflict'
  ...
  TextColumn get serverVersionData =>
      text().nullable()(); // JSON of server version if conflict

  // Timestamps
  DateTimeColumn get localCreatedAt =>
      dateTime()(); // Client-side creation time
  DateTimeColumn get localUpdatedAt =>
      dateTime()(); // Client-side last modification
...
@TableIndex(
    ...
    columns: {#userId, #syncStatus, #nextRetryAt})
...
class SyncQueueItems extends Table {
  ...
  TextColumn get entityId =>
      text()(); // UUID of offline expense or server expense ID
  TextColumn get payload => text()(); // JSON serialized data for the operation

  // Sync Metadata
  TextColumn get syncStatus =>
      text()(); // 'pending', 'syncing', 'completed', 'failed'
  IntColumn get retryCount => integer().withDefault(const Constant(0))();
...
class OfflineExpenseImages extends Table {
  ...
  TextColumn get expenseId =>
      text()(); // References offline_expenses(id) or synced server expense ID
  TextColumn get userId => text()();
  ...
  TextColumn get uploadStatus =>
      text()(); // 'pending', 'uploading', 'completed', 'failed'
  TextColumn get storagePath =>
      text().nullable()(); // Supabase storage path after upload
...
class SyncConflicts extends Table {
  ...
  TextColumn get conflictType =>
      text()(); // 'update_conflict', 'delete_conflict'

  // Resolution
  TextColumn get resolutionAction =>
      text().nullable()(); // 'accepted_server', 'created_new', 'discarded'
  ...
  MigrationStrategy get migration => MigrationStrategy(
        ...
        onUpgrade: (Migrator m, int from, int to) async {
          ...
          if (from < 4) {
            ...
            await m.addColumn(
                offlineExpenses, offlineExpenses.recurringExpenseId);
            await m.addColumn(
                offlineExpenses, offlineExpenses.isRecurringInstance);
          }
          if (from < 5) {
            // Add sortOrder to CachedCategories
            await m.addColumn(cachedCategories, cachedCategories.sortOrder);
          }
          if (from < 6) {
            // Add isActive to CachedCategories
            await m.addColumn(cachedCategories, cachedCategories.isActive);
          }
        },
      );
